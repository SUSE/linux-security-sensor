...
func NewJournalWithFlags(flags int) (j *Journal, err error) {
	...
	if cflags == 0 {
		cflags = C.SD_JOURNAL_LOCAL_ONLY
		fmt.Printf("Using default flags %v\n", cflags)
	}

	r := C.my_sd_journal_open(sd_journal_open, &j.cjournal, cflags)

	if r < 0 {
		return nil, fmt.Errorf("failed to open journal with flags %x: %s", flags, syscall.Errno(-r).Error())
	}
...
func NewJournal() (j *Journal, err error) {
	return NewJournalWithFlags(0)
}

// NewJournalFromDir returns a new Journal instance pointing to a journal residing
// in a given directory.
func NewJournalFromDirWithFlags(path string, flags int) (j *Journal, err error) {
	j = &Journal{}
	...
	r := C.my_sd_journal_open_directory(sd_journal_open_directory, &j.cjournal, p, cflags)
	if r < 0 {
...
func NewJournalFromDir(path string) (j *Journal, err error) {
	return NewJournalFromDirWithFlags(path, 0)
}
