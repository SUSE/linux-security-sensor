...
type ListenerBytesBuf interface {
	Data() []byte
	Get()
	Put()
}
...
func (self *ListenerBytes) Send(buf ListenerBytesBuf) {
	buf.Get()
	...
	if self.file_buffer_active {
		self.file_buffer.EnqueueSerialized(buf.Data())
		buf.Put()
		return
	}
...
func (self *ListenerBytes) Output() chan ListenerBytesBuf {
	return self.output
}
...
func (self *internalBuf) Data() []byte {
	return self.data
}

func (self *internalBuf) Get() {
}

func (self *internalBuf) Put() {
}
...
func NewListenerBytes(
	...
	self := &ListenerBytes{
		id:      utils.GetId(),
		name:    name,
		output:  make(chan ListenerBytesBuf),
		ctx:     subctx,
		cancel:  cancel,
		options: options,
	}
