...
type LogScaleQueueStats struct {
...
// Yes, this is inherently racy but it's not critical that the numbers add up
// perfectly.
func (self *LogScaleQueue) GetStats() LogScaleQueueStats {
	return LogScaleQueueStats {
		CurrentQueueDepth: int(atomic.LoadInt64(&self.currentQueueDepth)),
		QueuedBytes:       int(self.listener.FileBufferSize()),
		PostedEvents:      int(atomic.LoadInt64(&self.postedEvents)),
		PostedBytes:       int(atomic.LoadInt64(&self.postedBytes)),
		DroppedEvents:     int(atomic.LoadInt64(&self.failedEvents)),
		TotalRetries:      int(atomic.LoadInt64(&self.totalRetries)),
	}
}
