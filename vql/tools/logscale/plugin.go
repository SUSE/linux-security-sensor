...
func StatsDelta(now, then *LogScaleQueueStats) LogScaleQueueStats {
	stats := *now

	// Queue state isn't accumulative

	stats.DroppedEvents = now.DroppedEvents - then.DroppedEvents
	stats.PostedEvents = now.PostedEvents - then.PostedEvents
	stats.PostedBytes = now.PostedBytes - then.PostedBytes
	stats.FailedEvents = now.FailedEvents - then.FailedEvents
	stats.TotalRetries = now.TotalRetries - then.TotalRetries
...
func (self logscalePlugin) Call(ctx context.Context,
	...
	go func() {
		...
		if arg.StatsInterval > 0 {
			interval := time.Duration(arg.StatsInterval) * time.Second
		...
		for {
			...
			case <-tickerChan:
				...
				stats := StatsDelta(&newStats, &oldStats)
				statsRow := ordereddict.NewDict().
						Set("QueueDepth",   stats.CurrentQueueDepth).
						Set("QueuedBytes",   stats.QueuedBytes).
						Set("PostedEvents", stats.PostedEvents).
						Set("PostedBytes",  stats.PostedBytes).
						Set("FailedEvents", stats.FailedEvents).
						Set("TotalRetries", stats.TotalRetries)
				outputChan <- statsRow
				oldStats = newStats
			}
		}
