...
type JournalReaderConfig struct {
	...
	Path string
	// If nonzero, the specified flags will be used to open the journal
	OpenFlags   int
...
func NewJournalReader(config JournalReaderConfig) (*JournalReader, error) {
	...
	var err error
	if config.Path != "" {
		r.journal, err = NewJournalFromDirWithFlags(config.Path, config.OpenFlags)
	} else {
		r.journal, err = NewJournalWithFlags(config.OpenFlags)
	}
	if err != nil {
		return nil, err
	}
