...
func assertMUserExists(user, filepath string, s *CronSnooper, t *testing.T) {
	if _, ok := s.system_cron_registry[filepath][user]; !ok {
		t.Fatal("No cron entry for user", user)
	}
}

func assertMUserDoesntExist(user, filepath string, s *CronSnooper, t *testing.T) {
	if _, ok := s.system_cron_registry[filepath][user]; ok {
		t.Fatal("Cron entries present for user", user)
	}
}

func assertMUserCmdCount(user, filepath string, expected int, s *CronSnooper, t *testing.T) {
...
func createNamedFile(tempdir, filename string, content []string, t *testing.T) string {
