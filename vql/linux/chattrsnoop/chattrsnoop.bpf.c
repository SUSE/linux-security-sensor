...
static __always_inline int process_dentry(struct buf_t *string_p, int buf_off,
...
static __always_inline u32 get_path_str(struct path *path, struct buf_t *string_p)
{
		...
		struct buf_t *string_p)
{
    ...
    for (int i = 0; i < MAX_PATH_COMPONENTS; i++) {
        struct dentry *d_parent = BPF_CORE_READ(dentry, d_parent);
        if (dentry == d_parent) {
			bpf_printk("breaking due to dentry");
            break;
        }
		...
		unsigned int cmd, unsigned long arg)
{
    ...
    return bpf_perf_event_output(ctx, &events, BPF_F_CURRENT_CPU, &buf->buf[IDX(offset)], str_len);
}

#if 0
SEC("kprobe/vfs_fileattr_set")
int BPF_KPROBE(trace_vfs_fileattr_set, struct user_namespace *mnt_userns,
		struct dentry *dentry, struct fileattr *fa)
{
	...
	/* Is immutable state being changed at all */
	if (!(!!(flags & FS_IMMUTABLE_FL) ^ is_immutable))
		return 0;
    ...
    u32 offset = get_dentry_path_str(dentry, buf);
	offset -= 1;
